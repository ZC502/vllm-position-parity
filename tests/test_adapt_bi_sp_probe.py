...
spec = importlib.util.spec_from_file_location(
    "adapt_bi_sp_probe",
    ROOT / "tools" / "adapt_bi_sp_probe.py",
)
...
class AdapterTests(unittest.TestCase):
    ...
    def test_control_recomputes_all_zero(self):
        ...
        self.assertEqual(
            out["derived_observations"]["decode_steps_per_prompt_min"], 24
        )
        self.assertEqual(
            out["derived_observations"]["decode_steps_per_prompt_max"], 24
        )
    ...
    def test_failing_reproduces_source_and_splits_mismatch_semantics(self):
        ...
        for pair, values in expected.items():
            ...
            self.assertEqual(
                s["different_token_logprob_mismatch_positions"], values[2]
            )
